...
let tests =
  testList "TryParse" [
    testCase "DateTime kind" <| fun _ ->
      ...
      Expect.equal expected dt.Kind "Manually initialized datetime should equal"
    testCase "DateTime.FSharpPlus" <| fun _ ->
      let v1 : DateTime       = parse "2011-03-04T15:42:19+03:00"
      let expected =DateTime(2011,3,4,12,42,19)
      Expect.equal expected v1 "Manually initialized datetime should equal"
    testCase "DateTimeOffset.FSharpPlus" <| fun _ ->
      ...
      Expect.equal expected v2 "Manually initialized offset should equal"
